...
const App = () => {
  ...
  const fetchTasks = async () => {
    try {
      const responseToDo = await axios.get('https://alphansat-api1.onrender.com/api/tasks/To%20Do');
      setTasksToDo(responseToDo.data);

      const responseDoing = await axios.get('https://alphansat-api1.onrender.com/api/tasks/Doing');
      setTasksDoing(responseDoing.data);

      const responseDone = await axios.get('https://alphansat-api1.onrender.com/api/tasks/Done');
  ...
  const handleAddTask = async (newTask, status) => {
    try {
      if (editingTask) {
        await axios.patch(`https://alphansat-api1.onrender.com/api/tasks/${editingTask._id}`, { ...newTask, status });
        setEditingTask(null);
      } else {
        await axios.post('https://alphansat-api1.onrender.com/api/tasks', { ...newTask, status });
      }
  ...
  const onDragEnd = async (result) => {
    ...
    try {
      await axios.patch(`https://alphansat-api1.onrender.com/api/tasks/${draggedTask._id}`, {
  ...
  const handleDeleteTask = async (taskId, status) => {
    try {
      await axios.delete(`https://alphansat-api1.onrender.com/api/tasks/${taskId}`);
      fetchTasks();
    } catch (error) {
